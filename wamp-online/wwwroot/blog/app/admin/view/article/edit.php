                    ...
                    <div class="col-sm-9">
                        <select class="js-example-basic-single form-control" name="category_cid">
                            ...
                            <foreach from="$cateData" key="$k" value="$v">
                                <option value="{{$v['cid']}}" <if value="$oldData['category_cid']==$v['cid']">selected</if>>{{$v['_cname']}}</option>
                            </foreach>
                        </select>
                    </div>
                </div>
                    ...
                    <div class="col-sm-9">
                        <foreach from="$tagData" key="$k" value="$v">
                            <label class="checkbox-inline">
                                <input type="checkbox"  name="tag_tid[]" value="{{$v['tid']}}" <if value="in_array($v['tid'],$arcTag)">checked</if>>
                                {{$v['tname']}}
                            </label>
                        </foreach>
                    </div>
...
<!--                    单图上传-->
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-9">
                        <textarea type="text" name="keywords"  class="form-control" placeholder="">{{$oldData['keywords']}}</textarea>
                    </div>
                </div>
                <div class="form-group">
                    <label for="" class="col-sm-2 control-label">文章描述</label>
                    <div class="col-sm-9">
                        <textarea type="text" name="des"  class="form-control" placeholder="">{{$oldData['des']}}</textarea>
                    </div>
                </div>
                <div class="form-group">
                    ...
                    <div class="col-sm-9">
<!--                        百度编辑器-->
                        <textarea id="container" style="height:300px;width:100%;" name="content">{{$oldData['content']}}</textarea>
                        <script>
