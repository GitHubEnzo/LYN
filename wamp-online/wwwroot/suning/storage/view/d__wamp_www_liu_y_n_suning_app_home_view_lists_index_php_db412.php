...
<div id="">
	<div id="recommend">
		<?php if(!empty($hot)){ foreach ($hot as $k=>$v){?>		
			<div>
...
<div id="">
	<div id="navbar">
		<a href="<?php echo u('entry.index')?>"><h3>首页></h3></a>
		<?php if(!empty($fatherData)){ foreach ($fatherData as $k=>$v){?>
			<a href="<?php echo u('lists.index',['cid'=>$v['cid']])?>"><h3><?php echo $v['cname']?>></h3></a>
		<?php }}?>		
	</div>
</div>
<!--导航条结束-->
...
<div class="area">
	<ul>
		...
		<div id="screen_right">
			...
			<?php if(!empty($finalTempAttr)){ foreach ($finalTempAttr as $k=>$v){?>				
				<div class="screen_right_top2_son">
					...
					<div class="">
						<a <?php if($param[$k] == 0){?>
                class="hover"
               <?php }?> href="<?php echo u('index',['cid'=>Q('get.cid'),'param'=>implode('_',$temp)])?>">不限</a>						
						<?php if(!empty($v['value'])){ foreach ($v['value'] as $kk=>$vv){?>
							<?php $temp[$k] = $vv['gtid'] ?>									
	 						<a <?php if($param[$k] == $vv['gtid']){?>
                class="hover"
               <?php }?> href="<?php echo u('index',['cid'=>Q('get.cid'),'param'=>implode('_',$temp)])?>"><?php echo $vv['gtvalue']?></a>									
						<?php }}?>
					</div>
				</div>	
				<?php }}?>			
			...
			<div id="goods">
			<?php if(!empty($goodsData)){ foreach ($goodsData as $k=>$v){?>
				<div class="goods">
					<div class="goods_tp">
...
<link rel="stylesheet" type="text/css" href="<?php echo __ROOT__?>/public/css/lists.css"/>
<style>
		.hover{
			background: #FF7700;
			color: white;
